...
function toStatusKey(name = '') {
  ...
  if (['done','完了','completed','closed','cone'].some(k => n.includes(k))) return 'done';
  if (['review','レビュー','in review'].some(k => n.includes(k)))           return 'review';
  if (['in progress','進行中','doing','wip'].some(k => n.includes(k)))      return 'inprogress';
  return 'todo';
}
